...
namespace Com.Ctrip.Framework.Apollo.Internals
{
    public class LocalFileConfigRepository : AbstractConfigRepository, IRepositoryChangeListener
    {
        ...
        internal const string CONFIG_DIR = "config-cache";
        private readonly string m_namespace;
        private string m_baseDir;
        private readonly ConfigUtil m_configUtil;
        private volatile Properties m_fileProperties;
        private volatile IConfigRepository m_upstream;

        public LocalFileConfigRepository(string namespaceName) : this(namespaceName, null)
        {
        }

        public LocalFileConfigRepository(string namespaceName, IConfigRepository upstream)
        {
        ...
        {
            var localConfigDir = string.Empty;
            if (ConfigService.ApolloConfigSection == null || string.IsNullOrWhiteSpace(localConfigDir = ConfigService.ApolloConfigSection.LocalConfigDir))
            {
                try
                {
            ...
            else
            {
                try
                {
                    m_baseDir = Path.Combine(localConfigDir, CONFIG_DIR);
                }
                catch (Exception ex)
                {
                    logger.Warn(new ApolloConfigException("Prepare config cache dir failed", ex));

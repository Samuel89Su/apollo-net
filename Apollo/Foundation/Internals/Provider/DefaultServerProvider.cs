...
namespace Com.Ctrip.Framework.Foundation.Internals.Provider
{
    class DefaultServerProvider : IServerProvider
    {
        ...
        public void Initialize()
        {
            try
            {
                if (ConfigService.ApolloConfigSection != null && ConfigService.ApolloConfigSection.Env != null && !string.IsNullOrWhiteSpace(ConfigService.ApolloConfigSection.Env))
                {
                    serverProperties[KEY_ENV] = ConfigService.ApolloConfigSection.Env;
                    Initialize(null);
                }
                else if (System.IO.File.Exists(SERVER_PROPERTIES_FILE))
                {
        ...
        private void InitEnvType()
        {
            ...
            serverProperties.TryGetValue(KEY_ENV, out env);
            if (!String.IsNullOrWhiteSpace(env))
            {
                env = env.Trim();
